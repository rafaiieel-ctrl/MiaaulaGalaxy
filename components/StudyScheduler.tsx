...
interface StudySchedulerProps {
  questions: Question[];
  onStartStudy: (title: string, questions: Question[]) => void;
}

const CalendarDay: React.FC<{
  date: Date;
  questionsDue: Question[];
  isCurrentMonth: boolean;
  onSelectDate: (date: Date) => void;
}> = ({ date, questionsDue, isCurrentMonth, onSelectDate }) => {
  const day = date.getDate();
  const count = questionsDue.length;
  ...
  if (!isCurrentMonth) {
      ...
      bgClass = isPast ? "bg-rose-500/5" : "bg-white/5";
      textClass = isPast ? "text-rose-400" : "text-slate-200";
  }
  ...
  return (
    <button 
      ...
      className={`
        ...
        ${count > 0 ? 'cursor-pointer active:scale-95' : 'cursor-default'}
      `}
    >
...
const ScheduledSessionModal: React.FC<{
  ...
  date: Date | null;
  questions: Question[];
  onStartSession: (title: string, questions: Question[]) => void;
}> = ({ isOpen, onClose, date, questions, onStartSession }) => {
    const { settings } = useSettings();

    if (!isOpen || !date) return null;

    const handleStart = () => {
        const title = `Sessão Agendada: ${date.toLocaleDateString('pt-BR')}`;
        onStartSession(title, questions);
        onClose();
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-fade-in" onClick={onClose}>
            <div 
                className="bg-slate-900 border border-white/10 w-full max-w-2xl max-h-[80vh] rounded-3xl shadow-2xl flex flex-col" 
                onClick={(e) => e.stopPropagation()}
            >
                <header className="p-6 border-b border-white/5 flex justify-between items-center shrink-0">
                    <div>
                        <h2 className="text-lg font-black text-white uppercase tracking-tight">Revisões para {date.toLocaleDateString('pt-BR')}</h2>
                        <p className="text-xs text-slate-400 mt-1 font-bold uppercase tracking-wider">{questions.length} Questões agendadas</p>
                    </div>
                     <button type="button" onClick={onClose} className="p-2 -mr-2 text-slate-400 hover:text-white transition-colors" aria-label="Fechar">✕</button>
                </header>
                <div className="p-4 overflow-y-auto space-y-2 custom-scrollbar">
                    {questions.map(q => (
                         <div key={q.id} className="p-3 bg-white/5 rounded-xl border border-white/5 flex justify-between items-center text-sm group hover:border-white/10 transition-colors">
                            <div className="min-w-0 pr-4">
                                <p className="font-bold text-white truncate">{q.questionRef}</p>
                                <p className="text-xs text-slate-500 font-medium uppercase tracking-wide truncate">{q.subject}</p>
                            </div>
                            <MasteryBadge score={srs.calculateCurrentDomain(q, settings)} lastAnswerAt={q.lastReviewedAt} size="sm" />
                        </div>
                    ))}
                </div>
                <footer className="p-4 border-t border-white/5 flex justify-end">
                    <button onClick={handleStart} className="bg-sky-600 text-white font-black uppercase tracking-widest text-xs py-3 px-8 rounded-xl shadow-lg hover:bg-sky-500 transition-all active:scale-95">
                        Estudar Agora
                    </button>
                </footer>
            </div>
...
const StudyScheduler: React.FC<StudySchedulerProps> = ({ questions, onStartStudy }) => {
  const { settings } = useSettings();
  ...
  const scheduleData = useMemo(() => {
    ...
    questions.forEach(q => {
      ...
      const dateKey = q.nextReviewDate.split('T')[0];
      if (!map.has(dateKey)) {
        map.set(dateKey, []);
      }
  ...
  const questionsForSelectedDate = useMemo(() => {
    ...
    <>
        <GlassPanel className="p-6">
            <div className="flex justify-between items-center mb-6">
                <SectionHeader icon={<CalendarIcon />} title="Calendário" subtitle="Previsão SRS" />
            ...
            <div className="grid grid-cols-7 gap-1">
                {calendarGrid.map(({ date, isCurrentMonth }, index) => {
                    ...
                    const questionsDue = scheduleData.get(dateKey) || [];
                    return (
                        <CalendarDay
                            key={index}
                            date={date}
        ...
        <ScheduledSessionModal
            ...
            date={selectedDate}
            questions={questionsForSelectedDate}
