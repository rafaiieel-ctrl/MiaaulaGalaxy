...
interface QuestionTypeMasteryChartProps {
  data: ChartData[];
}
...
const QuestionTypeMasteryChart: React.FC<QuestionTypeMasteryChartProps> = ({ data }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="h-[300px] flex items-center justify-center text-sm text-bunker-500 dark:text-bunker-400">
        Nenhum dado de tipo de questão para exibir.
      </div>
    );
  }
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto">
        ...
        {data.map((d, i) => {
          ...
          const barHeight = (d.avgMastery / 100) * innerHeight;
...
export default QuestionTypeMasteryChart;
